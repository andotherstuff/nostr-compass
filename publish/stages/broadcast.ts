...
export function relayFloor(configured: number, requested?: number): number {
  if (configured < 1) throw new Error("At least one relay must be configured");
  const floor = requested ?? 2;
  if (!Number.isInteger(floor) || floor < 1) throw new Error("relay_floor must be a positive integer");
  return Math.min(floor, configured);
}

export async function broadcastIssue(issue: number, reallyBroadcast: boolean, options: { outDir?: string; relaysPath?: string; ledgerPath?: string; broadcaster?: Broadcaster; encodeLinks?: boolean } = {}): Promise<{ article_ok: number; announcement_ok: number }> {
  ...
  const cfg = JSON.parse(await readFile(relaysPath, "utf8")) as { relays: string[]; relay_floor?: number };
  const floor = relayFloor(cfg.relays.length, cfg.relay_floor);
  ...
  const send = async (name: "article" | "announcement", event: SignedEvent): Promise<RelayReceipt[]> => {
    ...
    const ok = receipts.filter((r) => r.ok).length;
  ...
  await writeAtomic(join(issueDir, "receipts.json"), JSON.stringify({ relay_floor: floor, article: articleReceipts, announcement: announcementReceipts }, null, 2));

  const first_published_at = Number(article.tags.find((t) => t[0] === "published_at")?.[1] ?? article.created_at);
  const entry: PublishedEntry = { issue, event_id: article.id, announcement_id: announcement.id, first_published_at, last_edited_at: article.created_at, banner_url: article.tags.find((t) => t[0] === "image")?.[1] ?? "", relays_ok: articleReceipts.filter((r) => r.ok).map((r) => r.relay), relays_fail: articleReceipts.filter((r) => !r.ok).map((r) => r.relay) };
  ...
  const relayHints = [...new Set([...entry.relays_ok, ...announcementReceipts.filter((r) => r.ok).map((r) => r.relay)])].slice(0, 4);
  ...
  return { article_ok: entry.relays_ok.length, announcement_ok: announcementReceipts.filter((r) => r.ok).length };
}
