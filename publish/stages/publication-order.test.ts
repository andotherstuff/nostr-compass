...
test("delayed Pages success resumes retained merge state and only then broadcasts", async () => {
  ...
  await recordEditionAuthorization(out, 42, authorization);
  await prepareDeployment(out, 42, pageUrl); await pinPullRequest(out, 42, { number: 7, head_sha: head, base_sha: base });
