...
async function makeEditorialApproval(out: string) {
  ...
  const ledgerPath = join(out, "selection-ledger.json"); const ledgerRaw = JSON.stringify({ schema_version: 1, source_pass_id: "pass-1", source_manifest_sha256: sha256(sourceManifestRaw), draft_sha256: sha256("content"), selection_policy: { minimum_score: 8, maximum_score: 10, require_no_zero_axis: true, fixed_item_cap: null, qualified_items_must_publish: true }, hard_gate_fields: ["primary_evidence", "in_window_progress", "nostr_surface", "continuity_delta"], score_axes: ["nostr_significance", "user_operator_impact", "novelty", "evidence_maturity", "explanatory_value"], editorial_sources: [], source_expansion: [], candidates: [], final: true }); await writeFile(ledgerPath, ledgerRaw);
