...
export type RecipientDisposition = "no_dm" | "unknown_identity" | "missing_identity" | "no_nip17_inbox" | "failed" | "confirmed";
export type CampaignRecipientInput = {
  key?: string;
  npub?: string;
  names: string[];
  disposition?: Exclude<RecipientDisposition, "failed" | "confirmed">;
  reason?: string;
};

function normalizedRecipients(recipients: CampaignRecipientInput[]) {
  return recipients.map((recipient) => ({
    key: recipient.key ?? recipient.npub,
    npub: recipient.npub,
    names: [...recipient.names].sort(),
...
export async function assertOutreachObligation(args: { outDir: string; issue: number; campaign: "review" | "podcast-invitation"; prUrl?: string; newsletterUrl?: string; podcastUrl?: string; recipients: { npub: string; names: string[] }[] }): Promise<{ obligation_sha256: string; access_receipt_sha256?: string }> {
  ...
  return { obligation_sha256: effect.payload_sha256, access_receipt_sha256: obligation.readiness_receipt_sha256 };
}

export async function prepareCampaign(args: { outDir: string; issue: number; identity: string; message: string; recipients: CampaignRecipientInput[]; binding?: unknown }): Promise<OutreachCampaign> {
  const manifest = normalizedRecipients(args.recipients);
  if (manifest.some((recipient) => !recipient.key)) throw new Error("Every campaign recipient requires an immutable key or npub");
  const intent = sha256(stableJson({ identity: args.identity, message: args.message, recipients: manifest, binding: args.binding ?? null }));
  return mutateJournalAsync(args.outDir, args.issue, (journal) => {
    ...
    for (const recipient of args.recipients) {
      const key = recipient.key ?? recipient.npub!;
      const terminal = recipient.disposition;
      recipients[key] = {
        npub: recipient.npub,
        names: recipient.names,
        effect: terminal
          ? { state: terminal === "no_dm" ? "confirmed" : "failed", intent_sha256: sha256(stableJson({ key, terminal })), receipts: {}, disposition: terminal, error: recipient.reason }
          : { state: "prepared", intent_sha256: "unprepared", receipts: {} },
      };
    }
    const campaign = { identity: args.identity, intent_sha256: intent, message: args.message, recipients };
    journal.outreach[args.identity] = campaign;
    return campaign;
  });
}
...
export async function reuseOrBuildRecipient(args: { outDir: string; issue: number; campaign: string; npub: string; intent: unknown; build: () => Promise<SignedEvent> }): Promise<SignedEvent> {
  ...
  return mutateJournalAsync(args.outDir, args.issue, async (journal) => {
    const recipient = journal.outreach[args.campaign]?.recipients[args.npub];
    if (!recipient) throw new Error("Recipient is not in canonical campaign");
    const effect = recipient.effect;
    ...
    if (effect.intent_sha256 === intentHash && effect.payload_path && effect.payload_sha256) {
      const bytes = await readFile(effect.payload_path);
      if (sha256(bytes) !== effect.payload_sha256) throw new Error("Outreach signed payload hash mismatch");
      return JSON.parse(bytes.toString()) as SignedEvent;
    }
    const event = await args.build();
    const path = join(args.outDir, String(args.issue), "outreach", args.campaign, `${args.npub}.json`);
    const bytes = JSON.stringify(event, null, 2);
    await writeAtomic(path, bytes);
    recipient.effect = { state: "prepared", intent_sha256: intentHash, payload_path: path, payload_sha256: sha256(bytes), event_id: event.id, receipts: {} };
    return event;
  });
}
...
export async function recordRecipientReadback(outDir: string, issue: number, campaign: string, npub: string, relay: string, found: boolean): Promise<void> {
  await mutateJournal(outDir, issue, (journal) => {
    const effect = journal.outreach[campaign].recipients[npub].effect;
    effect.readbacks ??= {};
    effect.readbacks[relay] = { found, recorded_at: new Date().toISOString() };
  });
}
export async function finishRecipient(outDir: string, issue: number, campaign: string, npub: string, state: "confirmed" | "failed" | "ambiguous", error?: string, disposition?: "no_nip17_inbox" | "failed" | "confirmed"): Promise<void> {
  await mutateJournalAsync(outDir, issue, async (j) => {
    const effect = j.outreach[campaign].recipients[npub].effect;
    if (state === "confirmed") {
      if (!effect.event_id || !effect.payload_path || !effect.payload_sha256 || sha256(await readFile(effect.payload_path)) !== effect.payload_sha256) throw new Error("Confirmed recipient requires the exact retained signed payload");
      if (!Object.values(effect.readbacks ?? {}).some((row) => row.found)) throw new Error("Confirmed recipient requires independent exact-event readback evidence");
    }
    effect.state = state;
    effect.disposition = state === "confirmed" ? "confirmed" : state === "failed" ? (disposition ?? "failed") : undefined;
    effect.error = error;
  });
}
