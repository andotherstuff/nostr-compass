    ...
    paragraphs.each do |p|
      ...
      p.gsub!($podcast_reference_mark) do |match|
        if in_review_club_section
          # special sections that don't have list items use the header as anchor
          current_title["podcast_slug"] = "#review-club" # to avoid duplicate anchor
          current_title["slug"] = "#nip-review-club"
        end
      ...
      if p.sub(/^#+\s*/, "") == headers[(current_header + 1) % headers.length()]
        current_header += 1
        in_review_club_section = headers[current_header] == "NIP Review Club"
