...
**A project may be omitted only when it was covered in the previous issue AND shipped nothing substantive.** Recency alone is never sufficient. The digest computes `recent_followup`, `substance_signals` and `suppression_allowed` so this is mechanical: `suppression_allowed` is true only when the project is a recent follow-up with zero substance markers, those entries sort last, and the digest labels them "a skip is defensible". Against #37's own fetch exactly 1 of 40 release-bearing projects qualified, and its release notes were a bare compare link; Nail carried five signals. If a run reports many suppressible projects, distrust the run. A substance signal is a reason to look, never a claim for prose.
