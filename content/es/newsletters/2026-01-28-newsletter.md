...
El SDK de TypeScript ([marmot-ts](https://github.com/marmot-protocol/marmot-ts)) está construyendo una aplicación de chat de referencia. [PR #37](https://github.com/marmot-protocol/marmot-ts/pull/37) añade creación/listado de grupos, gestión de paquetes de claves con flujos de publicar/transmitir/eliminar, e invitaciones por código QR. Un [PR abierto #38](https://github.com/marmot-protocol/marmot-ts/pull/38) por hzrd149 implementa persistencia de historial de mensajes con paginación. En whitenoise-rs, se fusionaron 8 PRs a master esta semana, destacando Add event id to user reaction que permite la eliminación de reacciones al exponer los IDs de eventos de reacción, y fix(message_aggregator/emoji_utils): improve is_valid_emoji to capture more valid emoji que amplía la validación de emojis para soportar bloques Unicode extendidos, indicadores regionales y secuencias de teclas. Otras actualizaciones incluyen almacenamiento de preferencias de idioma para localización del lado del cliente y eliminación de apodos predeterminados para dar a los clientes control total sobre los metadatos de registro.
...
- **[Trusted Relay Assertions](https://github.com/nostr-protocol/nips/pull/1534)** - La propuesta borrador para estandarizar puntuaciones de confianza de relays [que cubrimos la semana pasada](/es/newsletters/2026-01-21-newsletter/#nip-updates) continúa en discusión. El debate central se centra en si las puntuaciones de confianza deben ser "globales" (calculadas una vez para todos los usuarios) o "personalizadas" (relativas al grafo social de cada observador). Algoritmos estilo PageRank como [Trust Rank de nostr.band](https://trust.nostr.band/) y [GrapeRank](https://github.com/Pretty-Good-Freedom-Tech/graperank-nodejs) resisten ataques sybil dividiendo cualquier rango pasado a través de cuentas falsas por el tamaño de la granja de bots. Los críticos argumentan que las puntuaciones verdaderamente personalizadas son más precisas pero requieren cálculo costoso por usuario. La discusión también explora si usar DVMs para puntuación bajo demanda versus eventos de atestación kind 30382 pre-calculados que los clientes pueden cachear.
