...
## Six Years of Nostr Marches

[Last month's newsletter](/en/newsletters/2026-03-04-newsletter/#six-years-of-nostr-februaries) followed the protocol from the NIP-01 rewrite through the Damus wave to mesh and agent systems. March's [repository trail](https://github.com/nostr-protocol/nips/commits/master/) shows Nostr changing scale in a different way: two specification edits in 2021, a small tool-building culture in 2022, SDK and wallet infrastructure after the 2023 adoption shock, slower consensus work in 2024 and 2025, then products in 2026 that use Nostr underneath networking, software distribution, and games. The arc is from defining event behavior to composing mature pieces without asking users to think about the relay layer.

### March 2021: One Repository, One Field of View

Four months into its existence, Nostr still fit inside one small protocol repository. On March 4, fiatjaf [added links to nostwitter instances](https://github.com/nostr-protocol/nostr/commit/dcd8cc3), pointing readers of the specification to working deployments. That documentation change captures the first stage: protocol and application discovery lived together, and finding a client meant following links from the specification itself.

The important filter boundary had actually landed on February 15, when [commit 54dfb46 added `kind` to the basic filter definition](https://github.com/nostr-protocol/nostr/commit/54dfb466). By March, working nostwitter instances could build on that distinction between event types. The sequence matters more than a monthly commit count: a small filter primitive made it possible for later notes, contacts, payments, files, and application-specific events to share relays without being retrieved as one undifferentiated stream.
...
The [nostr-rs-relay repository](https://github.com/scsibug/nostr-rs-relay) shows the other side of that stage: the relay network and active user base were still small, and Damus and Nostream had not appeared. March's work was infrastructure for a developer community using its own tools. That quiet reliability work was what made the following month's client experiments possible; growth came after people could keep a relay and a daily-use client running.
...
March 2026 is the month where disparate threads converged into working systems. The [Marmot Development Kit](https://github.com/marmot-protocol/mdk) shipped its first public release with encrypted media and multi-language bindings; [Shopstr](https://github.com/shopstr-eng/shopstr) and Milk Market opened MCP commerce surfaces; and [NIP-42 relay authentication](https://github.com/nostr-protocol/nips/blob/master/42.md) landed across signer, relay, and bunker software. [Notedeck](https://github.com/damus-io/notedeck) also moved software-release discovery onto Nostr events. Pieces that had been separate specifications in earlier years were now being coordinated across complete product stacks.

The same composability appeared outside conventional clients. [BigBrotr](https://github.com/BigBrotr/bigbrotr) scanned relay data for leaked private keys and exposed a DVM checker, [nostr-vpn](https://github.com/mmalmi/nostr-vpn) used Nostr keys and relays for peer discovery before handing traffic to WireGuard, and [Vector's DOOM implementation](https://github.com/VectorPrivacy/Vector) combined Nostr discovery, Marmot encryption, and QUIC transport for a real-time game. These projects were interesting not because March produced many releases, but because Nostr had become one reusable coordination layer inside systems whose primary purpose was no longer posting notes.

The [NIPs commit history](https://github.com/nostr-protocol/nips/commits/master/) records the maintenance work underneath those products: wiki markup, bech32 input limits, boolean relay queries, and trusted assertions all changed during the month, while agent, WASM, and search proposals remained under review. The contrast with 2021 is the point. A filter-key addition once represented the whole month; by 2026 maintainers were refining many established contracts while applications combined them beyond the social-client category.

### Looking Ahead

Six Marches trace a clear arc from one maintainer adding a filter key to independent teams composing discovery, encryption, payments, storage, and transport. [NDK's first commit](https://github.com/nostr-dev-kit/ndk/commit/09e5e03) marks the middle of that transition: after the 2023 growth wave, reusable SDKs let later projects begin above the raw relay protocol. The surface area in 2026 would have been unrecognizable in 2021, but the signed-event and relay model remained stable enough that new work accumulated around it instead of forking it.
