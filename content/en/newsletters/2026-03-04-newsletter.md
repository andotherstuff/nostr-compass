...
## Six Years of Nostr Februaries

[Last month's newsletter](/en/newsletters/2026-01-28-newsletter/#six-years-of-nostr-januaries) traced Nostr's January milestones from early development through the Damus breakout to security infrastructure. February's [protocol history](https://github.com/nostr-protocol/nostr/commits/master/) follows a related but distinct progression: the wire model was rewritten in 2021, contributors and implementations multiplied in 2022, public attention forced rapid standardization in 2023, maintainers documented compatibility boundaries in 2024, wallets and infrastructure consolidated in 2025, and applications pushed beyond social feeds in 2026. Each stage begins where the previous February's bottleneck ended.
...
[NIP-04](https://github.com/nostr-protocol/nips/blob/master/04.md) arrived the same month, adding encrypted direct messages using shared secrets derived from Diffie-Hellman key exchange over secp256k1. Its encryption was basic (AES-256-CBC) and would later be replaced by [NIP-44](https://github.com/nostr-protocol/nips/blob/master/44.md)'s audited cryptography, but it gave the handful of early users their first private communication channel on the protocol.
...
Major outlets including [TechCrunch](https://techcrunch.com/2023/02/02/damus-pulled-from-apples-app-store-in-china-after-two-days/) and CoinDesk covered the removal, amplifying awareness of both the app and the protocol. Public-key and relay counts rose quickly enough that enthusiast-run infrastructure had to absorb a consumer-app growth event. That pressure explains why the same month produced an unusually dense specification cycle rather than merely more clients.
...
February 2024 shifted focus from growth to protocol durability. [NIP-17's pull request](https://github.com/nostr-protocol/nips/pull/716), open since the previous July, was working toward a replacement for the aging NIP-04 encryption using NIP-44's audited cryptography and NIP-59 gift wrapping. NIP-04 exposed sender-recipient metadata to relays; NIP-17 hid the sender behind disposable outer keys. The design work showed a network revisiting an early convenience after public use made its privacy cost concrete.

[NIP-29](/en/topics/nip-29/) (Simple Groups) [merged February 28](https://github.com/nostr-protocol/nips/pull/566) after months of discussion, defining how relays can host moderated group chats with admin roles and access control. [NIP-92](/en/topics/nip-92/) (imeta tags) merged February 1, standardizing how clients attach image dimensions and blurhash previews to media events. On February 16, the NIPs repository added [BREAKING.md](https://github.com/nostr-protocol/nips/commit/62c48eff), a file tracking backward-incompatible changes to the protocol specification. Its creation acknowledged that Nostr had reached a maturity level where breaking changes needed formal documentation.
...
February 2025 turned protocol growth into operational policy. [NIP-62's merge](https://github.com/nostr-protocol/nips/pull/1840) defined a signed request for relays to remove an author's stored events, while [Cashu wallet events](https://github.com/nostr-protocol/nips/blob/master/60.md), nutzaps, and quote tags were simplified around behavior clients were already implementing. These were not headline-grabbing new social primitives; they were the compatibility and lifecycle rules a relay ecosystem needs after users begin treating events as durable state and money.

The implementation layer was maturing at the same time. [Notedeck](https://github.com/damus-io/notedeck) carried its alpha into broader testing, [GRAIN](https://github.com/0ceanSlim/grain) shipped a Go relay release, and the first Nostr-to-MCP bridges connected [NIP-90 job events](https://github.com/nostr-protocol/nips/blob/master/90.md) to agent tooling. By this stage Nostr was no longer expanding only by adding event kinds: desktop clients, relay implementations, and external protocol bridges were becoming independently maintained product surfaces.

### February 2026: Beyond Social Media

February 2026, documented across Compass issues #8 through #11, produced the broadest application range in this sequence. [Mostro](https://github.com/MostroP2P/mostro) shipped its first public beta for decentralized peer-to-peer Bitcoin trading, [Zapstore](https://github.com/zapstore/zapstore) reached 1.0 after its release-candidate cycle, and [White Noise](https://github.com/marmot-protocol/whitenoise) delivered real-time Marmot-encrypted messaging with external signer support. Mature protocol pieces were now being assembled into products with trading, distribution, and private-group requirements rather than demonstrated in isolation.

Competing AI-agent proposals arrived alongside a DVM coordination design extending [NIP-90](https://github.com/nostr-protocol/nips/blob/master/90.md). [ContextVM](https://github.com/ContextVM) connected Model Context Protocol tooling to Nostr transport, while [Burrow](https://github.com/erskingardner/burrow) applied Marmot-encrypted messaging to both agents and humans. The notable transition was architectural: identity, payments, job requests, and private channels could now be composed for machine-to-machine work without inventing a separate account or transport system.

[FIPS](https://github.com/jmcorgan/fips) shipped a working Rust implementation of Nostr-native mesh networking, using secp256k1 keypairs as node identities with transport-agnostic routing over UDP, Ethernet, Bluetooth, or LoRa radio. Its design showed that Nostr's key model extends beyond social media into physical network infrastructure.
...
Six Februaries show a progression from wire-format foundations to developer growth, public pressure, compatibility discipline, infrastructure products, and new application domains. The turning point is visible between the 2021 [NIP-01 rewrite](https://github.com/nostr-protocol/nostr/commit/33a1a70) and 2026's mesh and agent systems: public adoption did not replace the early EVENT/REQ/CLOSE model. It forced maintainers to add privacy, lifecycle, routing, and composition rules around it until the same event network could support work far beyond a social timeline.
