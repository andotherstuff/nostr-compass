...
Open PRs include [NIP-46](/en/topics/nip-46/) QR code authentication ([PR #1019](https://github.com/divinevideo/divine-mobile/pull/1019)) and [NIP-17](/en/topics/nip-17/) encrypted direct messaging ([PR #834](https://github.com/divinevideo/divine-mobile/pull/834)). This week's activity focused on [mentions support](https://github.com/divinevideo/divine-mobile/pull/1098) converting `nostr:` URIs and @mentions to clickable profile links, [Classic Viners avatar fallbacks](https://github.com/divinevideo/divine-mobile/pull/1097) using Nostr profiles, and video editing tools including [drawing](https://github.com/divinevideo/divine-mobile/pull/1056), [filters](https://github.com/divinevideo/divine-mobile/pull/1053), and [stickers](https://github.com/divinevideo/divine-mobile/pull/1050).
...
- **[Trusted Relay Assertions](https://github.com/nostr-protocol/nips/pull/1534)** - The proposal for standardizing relay trust scoring [we covered last week](/en/newsletters/2026-01-21-newsletter/#nip-updates) was merged. The spec defines kind 30385 events for relay trust assertions with scoring across reliability, quality, and accessibility. The discussion leading to merge centered on whether trust scores should be "global" (computed once for all users) or "personalized" (relative to each observer's social graph). PageRank-style algorithms like [nostr.band's Trust Rank](https://trust.nostr.band/) and [GrapeRank](https://github.com/Pretty-Good-Freedom-Tech/graperank-nodejs) resist sybil attacks by dividing any rank passed through fake accounts by the size of the bot farm.
...
The technical foundation was already locked in the early [`nostr-protocol/nostr` specification](https://github.com/nostr-protocol/nostr): users identified by secp256k1 public keys, posts cryptographically signed with Schnorr signatures, and relays serving as independent storage rather than a federated cluster. This deliberately small, Bitcoin-native base mattered because the network could add clients and relays without reopening identity or event authenticity first.
...
January refined rather than introduced NIP-59. A [January 9 clarification](https://github.com/nostr-protocol/nips/commit/1a2b21b6) and [January 31 fix](https://github.com/nostr-protocol/nips/commit/f3589b99) tightened gift-wrapping behavior after the proposal had entered the repository in August 2023. Gift Wrap builds on [NIP-44's encryption standard](https://github.com/paulmillr/nip44) to hide sender identity from relays by wrapping an encrypted message inside an outer event signed by a random, one-time-use keypair. Relays see only the disposable pubkey, while the real sender's identity is buried in the encrypted payload that only the recipient can decrypt. This prevents relay operators and network observers from learning who is messaging whom. Timestamps can also be randomized to defeat timing analysis.
...
Yet infrastructure sustainability [remained challenging](https://arxiv.org/abs/2402.05709). Academic research from this period found 95% of relays struggled to cover operational costs, with 20% experiencing significant downtime, while paid-relay admission fees remained too small to fund most operators. The same visibility also attracted projects trading on Nostr's name: fiatjaf [publicly rejected the unrelated Nostr Assets Protocol](https://www.aicoin.com/en/article/377704) as an affinity scam. Growth had created both an operations problem and a credibility problem that the protocol document alone could not solve.
...
Major clients began moving away from NIP-04 direct messages toward the metadata-protecting design in [NIP-17](https://github.com/nostr-protocol/nips/blob/master/17.md), while the [NIP-65 relay-list model](https://github.com/nostr-protocol/nips/blob/master/65.md) gave clients a shared basis for outbox and inbox routing. January 2025 was therefore less about one breakout app than convergence: mature clients were replacing early shortcuts with portable encryption and relay-selection contracts, work that made the later multi-device and signer layers possible.
...
Protocol work moved toward security infrastructure shared by multiple clients. The [Marmot Protocol](https://github.com/marmot-protocol/mdk) hardened its [MLS](/en/topics/mls/)-based encrypted messaging against audit findings, while signer and relay-trust discussions showed the same pressure elsewhere: security behavior was becoming cross-client plumbing rather than an application-local convention.
...
Six years of Januaries move from early development to discovery, breakout growth, protocol hardening, client convergence, and security infrastructure. The pattern is visible in the distance between the early [`nostr-protocol/nostr` repository](https://github.com/nostr-protocol/nostr) and 2026's audited signer, Marmot, and recovery work: once the relay/key/event base held, each stage pushed complexity away from user-held secrets and one-off client behavior into shared, reviewable interfaces. Nostr did not become mature by replacing its foundation; it became useful by learning where that foundation needed stronger contracts around it.
