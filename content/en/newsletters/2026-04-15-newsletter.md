...
The notification model supports both plaintext and [NIP-44](/en/topics/nip-44/) encrypted payloads. When encryption is enabled, nstrfy uses [Amber](https://github.com/greenart7c3/Amber) for signing via [NIP-55](/en/topics/nip-55/) or a local nsec. Topic-based subscriptions let users configure per-topic sender allowlists with npub whitelisting, so only approved senders can trigger notifications for a given topic. The app imports relay lists from the user's profile using [NIP-65](/en/topics/nip-65/) and respects [NIP-40](/en/topics/nip-40/) event expiration. The full ntfy notification vocabulary is supportedf - tap-to-open URLs, priority levels, custom icons, and action buttons — so most ntfy-style alerts translate directly. User search is powered by NIP-50 via [Web of Trust](/en/topics/web-of-trust/) data from brainstorm.world.

The [nstrfy.sh](https://github.com/vcavallo/nstrfy.sh) companion project provides both a bash CLI and a hosted web client at [nstrfy.sh](https://nstrfy.sh) for sending and listening from the browser, with NIP-07 signer support. The native app is available on [Zapstore](https://zapstore.dev/apps/io.nstrfy.android).
