...
TypeScript SDK（[marmot-ts](https://github.com/marmot-protocol/marmot-ts)）正在构建参考聊天应用。[PR #37](https://github.com/marmot-protocol/marmot-ts/pull/37) 添加了群组创建/列表、带有发布/广播/删除流程的密钥包管理和二维码邀请。hzrd149 的[开放 PR #38](https://github.com/marmot-protocol/marmot-ts/pull/38) 实现了带有分页的消息历史持久化。在 whitenoise-rs 上，本周有 8 个 PR 被合并到 master，其中最值得注意的是 Add event id to user reaction（通过暴露反应事件 ID 来实现反应删除），以及 fix(message_aggregator/emoji_utils): improve is_valid_emoji to capture more valid emoji（扩展了表情符号验证以支持扩展 Unicode 块、区域指示符和按键序列）。其他更新包括用于客户端本地化的语言偏好存储，以及移除默认昵称以让客户端完全控制注册元数据。
