...
TypeScript SDK（[marmot-ts](https://github.com/marmot-protocol/marmot-ts)）はリファレンスチャットアプリケーションを構築しています。[PR #37](https://github.com/marmot-protocol/marmot-ts/pull/37)は、グループ作成/リスト、公開/ブロードキャスト/削除フローを備えたキーパッケージ管理、QRコード招待を追加します。hzrd149による[オープンPR #38](https://github.com/marmot-protocol/marmot-ts/pull/38)は、ページネーション付きのメッセージ履歴永続化を実装します。whitenoise-rsでは、今週8つのPRがmasterにマージされました。特に注目すべきは、リアクションイベントIDを公開することでリアクション削除を可能にするAdd event id to user reactionと、拡張Unicodeブロック、地域インジケーター、キーキャップシーケンスをサポートするために絵文字バリデーションを拡張するfix(message_aggregator/emoji_utils): improve is_valid_emoji to capture more valid emojiです。その他の更新には、クライアントサイドローカライゼーションのための言語設定保存と、クライアントがサインアップメタデータを完全に制御できるようにするデフォルトのニックネーム削除が含まれます。
