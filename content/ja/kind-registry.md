...
イベントkindは、Nostrイベントを分類する整数です。このレジストリでは、すべての標準化されたkindを説明とそれを定義するNIPと共にリストしています。

**Kindの範囲**（[NIP-01](https://github.com/nostr-protocol/nips/blob/master/01.md)より）:
- **0-999**: 通常イベント（すべてのバージョンを保持）
- **1000-9999**: 通常イベント（続き）
- **10000-19999**: 置換可能イベント（pubkeyごとに最新のみ保持）
- **20000-29999**: 一時的イベント（保存されず、転送のみ）
- **30000-39999**: アドレス指定可能イベント（pubkey + kind + d-tagごとに最新を保持）
