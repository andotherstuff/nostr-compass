...
Das TypeScript SDK ([marmot-ts](https://github.com/marmot-protocol/marmot-ts)) entwickelt eine Referenz-Chat-Anwendung. [PR #37](https://github.com/marmot-protocol/marmot-ts/pull/37) fügt Gruppenerstellung/-auflistung, Key-Package-Management mit Publish/Broadcast/Delete-Flows und QR-Code-Einladungen hinzu. Ein [offener PR #38](https://github.com/marmot-protocol/marmot-ts/pull/38) von hzrd149 implementiert Nachrichtenverlaufs-Persistenz mit Paginierung. Bei whitenoise-rs wurden diese Woche 8 PRs in den Master gemerged, insbesondere „Add event id to user reaction", das die Löschung von Reaktionen durch Offenlegung von Reaktions-Event-IDs ermöglicht, und „fix(message_aggregator/emoji_utils): improve is_valid_emoji to capture more valid emoji", das die Emoji-Validierung erweitert, um erweiterte Unicode-Blöcke, Regionalindikatoren und Keycap-Sequenzen zu unterstützen. Weitere Updates umfassen die Speicherung von Sprachpräferenzen für clientseitige Lokalisierung und die Entfernung von Standard-Spitznamen, um Clients die volle Kontrolle über Signup-Metadaten zu geben.
...
- **[Trusted Relay Assertions](https://github.com/nostr-protocol/nips/pull/1534)** - Der Entwurfsvorschlag zur Standardisierung von Relay-Vertrauensbewertungen, [den wir letzte Woche vorgestellt haben](/de/newsletters/2026-01-21-newsletter/#nip-updates), wird weiter diskutiert. Die Kerndebatte dreht sich darum, ob Vertrauensbewertungen „global" (einmal für alle Benutzer berechnet) oder „personalisiert" (relativ zum sozialen Graphen jedes Beobachters) sein sollten. PageRank-ähnliche Algorithmen wie [nostr.band's Trust Rank](https://trust.nostr.band/) und [GrapeRank](https://github.com/Pretty-Good-Freedom-Tech/graperank-nodejs) widerstehen Sybil-Angriffen, indem sie jeden durch gefälschte Konten weitergegebenen Rang durch die Größe der Bot-Farm teilen. Kritiker argumentieren, dass wirklich personalisierte Bewertungen genauer sind, aber teure Berechnungen pro Benutzer erfordern. Die Diskussion erforscht auch, ob DVMs für On-Demand-Bewertungen verwendet werden sollen im Vergleich zu vorberechneten kind 30382 Attestation-Events, die Clients cachen können.
