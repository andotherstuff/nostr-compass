...
class SourceRunManifestTests(unittest.TestCase):
    def test_immutable_pass_scoped_complete_receipt(self):
        with TemporaryDirectory() as directory:
            ...
            MODULE.record_family(manifest, pass_id="pass-001", family="projects", status="complete", artifact=artifact, collector=collector, query=query, pagination_complete=True, item_count=1, page_count=1, include_count=1, skip_count=0, skip_evidence=[])
    ...
    def test_fails_closed_on_window_pagination_and_skip_evidence(self):
        with TemporaryDirectory() as directory:
            ...
            with self.assertRaises(ValueError): MODULE.record_family(manifest, pass_id="pass-003", family="projects", status="complete", artifact=artifact, collector=collector, query=query, pagination_complete=False, item_count=1, page_count=1, include_count=0, skip_count=1, skip_evidence=[])
