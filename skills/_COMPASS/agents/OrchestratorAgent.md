...
Pass the full fetch result set, the activity digest, the intake list, and the durable `data/newsletter_workspace/recap_followup.yml` backlog. TriageAgent walks every release, every merged-PR project regardless of PR count, every NIP PR, every Zapstore new-app entry, every NIP-34 patch, every Nostr Recap event, and every still-pending Recap follow-up. For each item it writes one of: `GREEN` (keep), `MAYBE` (needs follow-up), `SKIP` (drop, with reason). It must persist newly unresolved Recap discoveries to `recap_followup.yml`; prose-only “carry forward” notes do not satisfy this stage.

For the exact project-updates file in the finalized source manifest, generate `project_activity_decisions_<date>.json` with `python3 scripts/project_activity_coverage.py --updates <project-updates> --template data/newsletter_workspace/project_activity_decisions_<date>.json` if it does not already exist. Triage completes every project row and runs `python3 scripts/project_activity_coverage.py --updates <project-updates> --decisions data/newsletter_workspace/project_activity_decisions_<date>.json`. Also run `python3 scripts/check_triage_coverage.py --digest data/project_updates/release_digest_<date>.json --triage data/newsletter_workspace/triage_<date>.md --strict` for tagged releases. Do not overwrite a prior decision file on resume; reconcile it with the immutable input. A pending row, a skipped 8/10 qualifier, or an untriaged release fails Stage 3.
